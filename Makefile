...
build: fetch_dependencies ## Build
	go build $(add_metadata) -v github.com/mp4096/clerk/cmd/clerk

install: fetch_dependencies ## Build and install
	go install $(add_metadata) -v github.com/mp4096/clerk/cmd/clerk

xcompile_linux: fetch_dependencies ## Cross-compile for Linux x64
	env GOOS=linux GOARCH=amd64 go build $(add_metadata) -v github.com/mp4096/clerk/cmd/clerk

xcompile_win: fetch_dependencies ## Cross-compile for Windows x64
	env GOOS=windows GOARCH=amd64 go build $(add_metadata) -v github.com/mp4096/clerk/cmd/clerk

xcompile_mac: fetch_dependencies ## Cross-compile for macOS x64
	env GOOS=darwin GOARCH=amd64 go build $(add_metadata) -v github.com/mp4096/clerk/cmd/clerk
